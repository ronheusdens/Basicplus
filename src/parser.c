...
/* Helper function: Check if a token type is a statement keyword */
static int is_statement_token(Token *tok)
{
    if (!tok)
        return 0;
...
/* Helper function: Parse a type keyword (INTEGER, STRING, DOUBLE, SINGLE) */
static VarType parse_type_keyword(Token *tok)
{
    if (!tok || !tok->value)
        return VAR_DOUBLE; /* default */

    const char *type_str = tok->value;

    if (strcasecmp(type_str, "INTEGER") == 0)
        return VAR_LONG; /* INTEGER -> VAR_LONG */
    if (strcasecmp(type_str, "STRING") == 0)
        return VAR_STRING;
    if (strcasecmp(type_str, "DOUBLE") == 0 || strcasecmp(type_str, "DWORD") == 0)
        return VAR_DOUBLE;
    if (strcasecmp(type_str, "SINGLE") == 0)
        return VAR_DOUBLE; /* For now, treat SINGLE as DOUBLE */
...
static ASTParameterList *parse_parameter_list(Parser *parser)
{
    ...
    while (current_token(parser) && current_token(parser)->type != TOK_RPAREN)
    {
        ...
        char *param_name = xstrdup(param_tok->value);
        advance(parser);

        VarType type = VAR_DOUBLE; /* default */
        if (match(parser, TOK_AS))
        {
            Token *type_tok = current_token(parser);
...
static ASTStmt *parse_procedure_def(Parser *parser)
{
    if (!parser || !current_token(parser) || current_token(parser)->type != TOK_PROCEDURE)
    {
        return NULL;
    }
    ...
    char *proc_name = xstrdup(name_tok->value);
    advance(parser);

    if (!expect(parser, TOK_LPAREN, "Expected '(' after procedure name"))
    {
        free(proc_name);
        return NULL;
    }
    ...
    {
        ast_parameter_list_free(params);
        free(proc_name);
        return NULL;
    }

    /* Skip newline after procedure header */
    ...
    while (current_token(parser) && current_token(parser)->type != TOK_EOF)
    {
        ...
        if (tok->type == TOK_END)
        {
            ...
            if (next && next->type == TOK_PROCEDURE)
            {
                break; /* Exit the loop */
            }
        }
    ...
    /* Expect END PROCEDURE */
    if (current_token(parser) && current_token(parser)->type == TOK_END)
    {
        advance(parser);
        if (!expect(parser, TOK_PROCEDURE, "Expected PROCEDURE after END"))
        {
            ast_parameter_list_free(params);
            free(proc_name);
            return NULL;
    ...
    {
        parser_error(parser, "Expected END PROCEDURE");
        ast_parameter_list_free(params);
        free(proc_name);
        return NULL;
    }

    /* Create the procedure definition statement */
    ASTStmt *proc = ast_stmt_create(STMT_PROCEDURE_DEF);
    proc->var_name = proc_name;
...
Program *parse_program(Parser *parser)
{
    ...
    while (parser->pos < parser->num_tokens && !parser_has_error(parser))
    {
        ...
        else if (tok->type == TOK_PROCEDURE)
        {
            ...
            if (proc)
            {
                /* Store procedure as a ProgramLine with line_num=0 (special marker) */
                ProgramLine *pline = ast_program_line_create(0, proc);
                ast_program_add_line(prog, pline);
            }
            else if (parser_has_error(parser))
            {
        ...
        /* Check for traditional line number (backward compatibility) */
        ...
        /* Check for statement keyword without line number (Phase 1 Basic++) */
        else if (is_statement_token(tok))
        {
            ASTStmt *stmt = parse_statement(parser);
            if (stmt)
            {
                /* Store statement as a ProgramLine with line_num=0 (no line number) */
                ProgramLine *pline = ast_program_line_create(0, stmt);
                ast_program_add_line(prog, pline);
            }
            else if (parser_has_error(parser))
            {
                /* Error occurred, advance to next line */
                advance(parser);
            }
        }
        else
