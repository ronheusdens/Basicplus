...
static int execute_stmt_internal(ExecutionContext *ctx, ASTStmt *stmt)
{
    ...
    /* Trace hook: called before each statement execution */
    if (g_trace_state.enabled || g_trace_state.step_mode || trace_has_breakpoint(stmt->line_number))
    {
        trace_before_statement(ctx, stmt->line_number, stmt);
    }
...
static int execute_poke_stmt(ExecutionContext *ctx, ASTStmt *stmt)
{
    ...
    /* Check if address is in video memory range and validate */
    if (addr >= 0x3C00 && addr <= 0x437F)
    {
        int result = videomem_poke((uint32_t)addr, value);
        if (result < 0)
        {
            /* Illegal memory access error (code 200) - silently ignore for now */
            return 0;
        }
        int offset = addr - VIDEO_BASE;
        int row = offset / VIDEO_COLS;
        int col = offset % VIDEO_COLS;
        termio_put_char_at(row, col, (char)(value & 0xFF));
    }
    else if (addr < 0 || addr > 0x437F)
    {
        /* Address is outside valid memory range - silently ignore (not in video or user area) */
        return 0;
    }
    else
    {
        /* Valid user memory address (0 to 0x3BFF, 15359) */
