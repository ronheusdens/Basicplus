...
{
    /* Check if address is in video memory range (0x3C00-0x437F, 15360-17279) */
    if (addr >= 0x3C00 && addr <= 0x437F)
    {
        videomem_poke((uint32_t)addr, value);
        return;
    }
...
int runtime_peek(RuntimeState *state, int addr)
{
    ...
    if (addr >= 0x3C00 && addr <= 0x437F)
    {
        int result = videomem_peek((uint32_t)addr);
        return result >= 0 ? result : 0; /* Return 0 on error */
    }
