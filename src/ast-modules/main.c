...
static void run_interactive(void)
{
    ...
    videomem_init(); /* Initialize video memory (0x3C00-0x437F) */
    ...
    while (1)
    {
        ...
        /* Trace commands */
        if (starts_with_keyword(p, "TRON"))
        {
            trace_enable();
            continue;
        }

        if (starts_with_keyword(p, "TROFF"))
        {
            trace_disable();
            continue;
        }

        if (starts_with_keyword(p, "STEP"))
        {
            trace_step();
            continue;
        }

        if (starts_with_keyword(p, "CONT"))
        {
            trace_continue();
            continue;
        }

        if (starts_with_keyword(p, "BREAK"))
        {
            const char *rest = p + 5;
            ...
            if (isdigit((unsigned char)*rest))
            {
                ...
                trace_add_breakpoint(line_num);
            }
            else
            {
                termio_printf("?SYNTAX ERROR: BREAK line_number\n");
            }
            continue;
        }

        if (starts_with_keyword(p, "NOBREAK"))
        {
            const char *rest = p + 7;
            while (*rest && isspace((unsigned char)*rest))
                rest++;
            ...
            {
                int line_num = atoi(rest);
                trace_remove_breakpoint(line_num);
            }
            else if (*rest == '\0')
            {
                trace_clear_breakpoints();
            }
            else
            {
                termio_printf("?SYNTAX ERROR: NOBREAK [line_number]\n");
            }
            continue;
        }
