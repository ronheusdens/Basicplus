...
/*
 * Scope - for procedure local variables
 */
typedef struct Scope
{
    int scope_id;         /* Unique identifier for this scope */
    void *local_vars;     /* SymbolTable ptr (cast to void to avoid circular dependency) */
    struct Scope *parent; /* Parent scope for variable lookup chain */
} Scope;

/*
 * Scope stack - manages procedure scope chain
 */
typedef struct
{
    Scope **stack;     /* Array of scopes */
    int depth;         /* Current depth (0 = global scope) */
    int capacity;      /* Allocated capacity */
    int next_scope_id; /* Counter for unique scope IDs */
} ScopeStack;
...
/* Scope stack management (for procedures) */
